...
function SchemerGenerate()
  ...
  table.insert(schemer_cmds, { save_only=true, content="let g:colors_name = 'schemer'" })
...
function SchemerSave()
  file = io.open(schemer_plugindir.."/colors/schemer.vim", "w")
  file:write("\" Generated by SchemerSave\n")
  for _, cmd in ipairs(schemer_cmds) do
    if type(cmd) == 'table' then
      if cmd.save_only then file:write(cmd.content) end
    else
      file:write(cmd)
    end
    file:write("\n")
